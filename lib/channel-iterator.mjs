...
WebSocketChannel.prototype[Symbol.asyncIterator] = function () {
	...
	return {
		next() {
			...
			if (pending) {
				return Promise.reject(new Error("cannot call next() concurrently"))
			}
		...
		return(value) {
			// Always clear stale buffer/error even if already done (e.g. a
			// terminal item was buffered which set done=true before return() ran).
			buffer = null
			bufferError = null
			if (!done) {
				cleanup()
		...
		throw(err) {
			// Always clear stale buffer/error even if already done.
			buffer = null
			bufferError = null
			if (!done) {
				cleanup()
