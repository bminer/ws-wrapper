...
class WebSocketChannel {
	...
	removeListener(eventName, listener) {
		...
		else if(listener._wsWrapperChannelFuncs)
			return this._emitter.removeListener(eventName,
				listener._wsWrapperChannelFuncs[this]);
	}
	...
	_wrapListener(listener) {
		...
		if(typeof listener._wsWrapperChannelFuncs !== "object") {
			Object.defineProperty(listener, "_wsWrapperChannelFuncs", {
				"enumerable": false,
				"value": {}
			});
		}
		var wrapped = listener._wsWrapperChannelFuncs;
		if(!wrapped[this]) {
			wrapped[this] = function channelListenerWrapper(event)
			{
			...
			wrapped[this]._original = listener;
		}
