...
class WebSocketWrapper extends WebSocketChannel {
	constructor(socket, options) {
		...
		// Option to disable pendingSend
		this._pendingQueue = typeof options.pendingQueue == 'boolean' ? options.pendingQueue:true;
	...
	send(data, ignoreMaxQueueSize) {
		if(this.isConnected) {
			...
			if (ignoreMaxQueueSize || this._pendingSend.length < WebSocketWrapper.MAX_SEND_QUEUE_SIZE) {
	...
	_onMessage(msg) {
		try {
			...
			if (
				...
				let channel = msg.c == null ? this : this._channels[msg.c]
				if (!channel) {
					if (msg.i >= 0) {
						this._sendReject(
							msg.i,
	...
	_sendEvent(channel, eventName, args, isRequest) {
		...
		let data = { a: Array.prototype.slice.call(args) }
		if (channel != null) {
			data.c = channel
		}
		let request
		if (isRequest) {
			...
			request = new Promise((resolve, reject) => {
				let pendReq = (this._pendingRequests[data.i] = {
					resolve: resolve,
					reject: reject,
				})
