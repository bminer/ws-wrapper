...
class WebSocketWrapper extends WebSocketChannel {
	...
	_onMessage(msg) {
		try {
			...
			} else if (msg.x && typeof msg.i === "number") {
				this._debug("wrapper: Processing cancellation for request", msg.i)
				// Handle cancellation message - find the corresponding active request
				const activeRequest = this._activeRequests[msg.i]
				if (activeRequest) {
					this._debug(`wrapper: Aborting active request ${msg.i}`)
					activeRequest.abort()
					delete this._activeRequests[msg.i]
				} else {
					this._debug(
						`wrapper: No active request found for cancellation ${msg.i}`
					)
				}
			}
