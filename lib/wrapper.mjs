...
class WebSocketWrapper extends WebSocketChannel {
	 ...
	 * @param {Object} [options.messageCodec] - Custom wire encoder/decoder
	 * @param {Function} [options.messageCodec.encode] - Encode protocol objects before sending
	 * @param {Function} [options.messageCodec.decode] - Decode inbound message data before routing
	...
	constructor(socket, options) {
		...
		const messageCodec = options.messageCodec || {}
		this._messageEncode =
			typeof messageCodec.encode === "function"
				? messageCodec.encode
				: JSON.stringify
		this._messageDecode =
			typeof messageCodec.decode === "function"
				? messageCodec.decode
				: JSON.parse
	...
	_onMessage(msg) {
		...
		try {
			...
			if (!msg || typeof msg !== "object" || Array.isArray(msg)) {
				this._debug(
					"wrapper: Ignoring decoded message: expected a non-null, non-array object"
				)
				return
			}
